...
const INTERVIEW_MODES = [
  { id: 'technical', label: 'Technical Interview', icon: Code, accent: 'primary' as const, desc: 'System design, algorithms & data structures' },
  ...
  { id: 'resume', label: 'Resume-Based', icon: FileText, accent: 'primary' as const, desc: 'AI extracts skills & generates tailored questions' },
  ...
  return (
    <Layout>
      <div className="space-y-8">
        {/* Hero */}
        <Card className="bg-blueprint bg-forge-glow relative overflow-hidden border-primary/15 p-8 md:p-10">
          <div className="relative z-10 flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
            <div>
              <div className="mb-3 flex items-center gap-3">
                <div className="rounded-2xl border border-primary/30 bg-primary/15 p-2.5">
            ...
            {INTERVIEW_MODES.map((mode) => {
              ...
              return (
                <button
                  ...
                  className="group relative rounded-2xl border border-border bg-card p-5 text-left transition-all duration-300 hover:-translate-y-1 hover:border-primary/30 hover:shadow-[0_14px_32px_-18px_hsl(var(--ember)/0.35)]"
                >
                  <div
                  ...
                  <ChevronRight className="absolute right-5 top-5 h-4 w-4 text-muted-foreground transition-colors group-hover:text-primary" />
                </button>
              );
            })}
          </div>
        ...
        {analytics && (
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            {[
              ...
              { label: 'Sessions', value: history.length, icon: Video, color: 'text-primary' },
            ...
            <div className="space-y-3">
              {history.slice(0, 5).map((s: any, idx: number) => (
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-2xl border border-border bg-card p-4 transition-colors hover:border-primary/30"
                >
