        ...
        >
          {STEPS.map(({ icon: Icon, label, desc }, i) => (
            <motion.div key={label} variants={motionConfig.fadeUp} className="relative group">
              <div className="flex h-12 w-12 items-center justify-center rounded-full border border-ember/30 bg-ember/10 font-mono text-sm text-ember transition-transform duration-300 group-hover:scale-110 group-hover:shadow-glow-primary">
              ...
              {i < STEPS.length - 1 && (
                <motion.div 
                  ...
                  className="absolute right-[-1rem] top-6 hidden h-px bg-ember md:block" 
                />
              )}
            </motion.div>
          ))}
