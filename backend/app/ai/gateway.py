...
class OmniRouteAIGateway:
    ...
    async def generate_question(
        self, req: QuestionGenerationRequest
    ) -> GeneratedQuestion:
        if self.base_url and self.api_key and self.api_key != "your-actual-api-key":
            ...
            try:
                ...
                logger.error(
                    f"Failed to call/parse online AI response for question generation: {e}"
                )
        ...
        prompt_text = difficulty_prompts.get(
            req.difficulty.lower(),
            f"Discuss the core design and application of {req.topic}.",
        )

        return GeneratedQuestion(
            prompt=prompt_text,
            expected_concepts=[
                f"{req.topic} architecture",
            ...
            citations=[
                f"Official {req.topic} Documentation",
                "StudyForge AI Knowledge Base",
            ],
    ...
    async def evaluate_answer(self, req: AnswerEvaluationRequest) -> AnswerEvaluation:
        ...
        matched = [
            c
            for c in req.expected_concepts
            if any(word in req.user_answer.lower() for word in c.lower().split())
        ]

        base_score = min(95.0, max(45.0, 55.0 + (text_len / 8.0)))
        concept_coverage = (
            (len(matched) / max(1, len(req.expected_concepts)))
            if req.expected_concepts
            else 0.8
        )
        ...
        if matched:
            strengths.append(
                f"Successfully addressed concepts: {', '.join(matched[:2])}"
            )
        else:
            strengths.append(
                "Demonstrated direct engagement with the practice question"
            )
        ...
        if unmatched:
            weaknesses.append(
                f"Omitted key expected concepts: {', '.join(unmatched[:2])}"
            )
        if text_len < 40:
            weaknesses.append(
                "Response could be expanded with more technical specifics and concrete code examples"
            )
        ...
        return AnswerEvaluation(
            ...
            weaknesses=(
                weaknesses
                if weaknesses
                else ["Minor formatting polish could further improve clarity"]
            ),
    ...
    async def next_interview_turn(
        ...
        question = (
            interview_questions[turn_count]
            if turn_count < len(interview_questions)
            else f"Question {turn_count + 1}: How do you handle technical debt in fast-moving projects?"
        )

        return InterviewTurnResponse(
            interviewer_question=question,
            feedback_on_previous=(
                "Good depth and structural methodology." if req.user_answer else None
            ),
            is_completed=False,
    ...
    async def _call_ai(self, prompt: str) -> str:
        ...
        if resp.status_code == 429 or resp.status_code >= 500:
            ...
            raise RetryableAIError(
                f"AI API request failed: {resp.status_code} - {resp.text}"
            )
