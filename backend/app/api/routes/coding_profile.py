...
async def create_profile(
    profile_in: CodingProfileCreate,
    ...
    slug_taken = await CodingProfile.find_one(CodingProfile.profile_slug == profile_in.profile_slug)
    if slug_taken:
...
async def connect_platform(
    ...
    if platform_name == "leetcode":
        ...
        if is_valid:
            profile.platforms.leetcode = PlatformConnection(username=req.username, verified=True)
    elif platform_name == "codeforces":
        is_valid = await verify_codeforces_username(req.username)
        if is_valid:
            profile.platforms.codeforces = PlatformConnection(username=req.username, verified=True)
    elif platform_name == "github":
        is_valid = await verify_github_username(req.username)
        if is_valid:
            profile.platforms.github = PlatformConnection(username=req.username, verified=True)
    else:
        raise HTTPException(status_code=400, detail="Unsupported platform")
        
    if not is_valid:
        raise HTTPException(status_code=400, detail=f"Username {req.username} not found on {platform_name}")
