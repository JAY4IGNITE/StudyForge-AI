...
async def analyze_resume(
    request: AtsAnalyzeRequest, current_user: User = Depends(get_current_user)
) -> Any:
    ...
    jd: JobDescriptionSchema = await job_parser_service.parse_job_description(
        request.job_text
    )
...
async def get_ats_history(current_user: User = Depends(get_current_user)) -> Any:
    """Get ATS report history for the user."""
    reports = (
        await AtsReport.find(AtsReport.user_id == str(current_user.id))
        .sort("-created_at")
        .to_list()
    )
    return reports
