...
async def create_session(
    req: StartSessionRequest, user: User = Depends(get_current_user)
):
    return await practice_service.create_session(
        user_id=str(user.id), topic_id=req.topic_id, target_role=user.target_role
    )
...
async def generate_next_question(
    ...
    return await practice_service.generate_next_question(
        session_id=session_id, user_id=str(user.id), target_role=user.target_role
    )


@router.post("/sessions/{session_id}/attempts")
async def submit_attempt(
    session_id: str, req: SubmitAnswerRequest, user: User = Depends(get_current_user)
):
