...
from app.schemas.resume import ResumeUploadUrlRequest, ResumeUploadUrlResponse, ResumeResponse, ResumeListResponse
...
async def generate_upload_url(
    request: ResumeUploadUrlRequest,
    ...
    r2_key = f"resumes/{current_user.id}/{resume_id}/original/{request.original_filename}"
...
async def process_resume(
    ...
    if parsed_resume.parse_quality == 0.0:
        ...
        resume.parse_status = "failed"
        await resume.save()
        raise HTTPException(status_code=400, detail="Parsing failed: " + str(parsed_resume.warnings))
