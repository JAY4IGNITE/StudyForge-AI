...
async def stream_text_chunks(
    text: str, websocket: WebSocket, chunk_size: int = 5, delay: float = 0.05
):
    ...
    for i in range(0, len(words), chunk_size):
        ...
        await websocket.send_json(
            WebSocketMessage(
                type="ai.stream.chunk", payload={"text": chunk}
            ).model_dump()
        )
...
async def _handle_interview_websocket(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        await manager.send_personal_message(
            WebSocketMessage(
                type="connection.connected", payload={"session_id": session_id}
            ),
            websocket,
        )
        ...
        while True:
            ...
            if event_type == "ping":
                await manager.send_personal_message(
                    WebSocketMessage(type="pong"), websocket
                )
                ...
                history = [
                    {"question": t.question, "user_answer": t.user_answer}
                    for t in session.turns
                ]

                # Notify UI that AI is processing
                await manager.send_personal_message(
                    WebSocketMessage(type="ai.stream.start", payload={}), websocket
                )
                ...
                try:
                    vision_telem = (
                        VisionTelemetry.model_validate(vision_metrics)
                        if vision_metrics
                        else None
                    )
                except Exception:
                ...
                try:
                    ...
                    await manager.send_personal_message(
                        WebSocketMessage(
                            ...
                            payload={
                                "code": "SERVER_ERROR",
                                "message": "Failed to save session turn",
                            },
                        ),
                        websocket,
                    )
