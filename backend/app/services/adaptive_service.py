...
class AdaptiveLearningService:
    ...
    async def get_or_create_mastery(user_id: str, topic_id: str) -> MasteryProfile:
        mastery = await MasteryProfile.find_one(
            MasteryProfile.user_id == user_id, MasteryProfile.topic_id == topic_id
        )
        if not mastery:
    ...
    @staticmethod
    async def update_mastery_after_attempt(
        user_id: str, topic_id: str, latest_score: float
    ) -> MasteryProfile:
        ...
        new_rolling_acc = (
            1 - alpha
        ) * mastery.rolling_accuracy + alpha * normalized_latest
        ...
        logger.info(
            f"Updated mastery for user {user_id} on topic {topic_id}: score={mastery.mastery_score}, diff={rec_diff}"
        )
        return mastery
