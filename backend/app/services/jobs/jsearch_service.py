...
class JSearchService:
    ...
    def __init__(self):
        self.api_key = (
            ...
            or "ak_vtwjdgusa9bfoeaerph11opdpbql6uow889t7zzr7yw3n7o"
        )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 429:
                    ...
                    raise HTTPException(
                        status_code=429,
                        detail="Job search rate limit exceeded. Please try again later.",
                    )
                ...
                if data.get("status") != "OK" and "data" not in data:
                    ...
                    raise HTTPException(
                        status_code=502,
                        detail="Failed to fetch jobs from external provider.",
                    )
            ...
            raise HTTPException(
                status_code=504, detail="Job search timed out. Please try again."
            )
        except httpx.HTTPStatusError as e:
            logger.error(
                f"JSearch API HTTP error: {e.response.status_code} - {e.response.text}"
            )
            raise HTTPException(
                status_code=502, detail="External job search service error."
            )
