...
class JobMatcher:
    ...
    def calculate_match(
        ...
        for req in job_req_lower:
            ...
            if any(
                req in user_skill or user_skill in req
                for user_skill in user_skills_lower
            ):
                # Find the original case from job requirements
                original_req = next(
                    (r for r in job_requirements if r.lower() == req), req
                )
        ...
        skill_score = (
            (len(matched) / len(job_requirements)) * 100 if job_requirements else 0
        )
        ...
        reasoning = (
            ...
            if final_score > 75
                ...
                if final_score > 40
                else "Significant skill gaps detected. Consider using the Study Roadmap to prepare."
            )
        )
