...
class ResumeParserService:
    ...
    async def parse_and_extract(
        resume_text: str, filename: str = "resume.pdf"
    ) -> Dict[str, Any]:
        """
...
"""
    ...
    def parse_pdf(file_bytes: bytes) -> Tuple[str, bool, float, List[str]]:
        ...
        try:
            ...
            if len(text.strip()) < 100 and len(doc) > 0:
                warnings.append(
                    "Insufficient text extracted from PDF. Document might be scanned."
                )
    ...
    def extract_sections(text: str) -> Dict[str, str]:
        ...
        for line in lines:
            ...
            if (
                len(line_clean) < 40
                and not any(char.islower() for char in line_clean)
                and len(line_clean.split()) < 5
            ):
    ...
    def parse_resume_file(
        ...
        if file_type == "pdf":
            raw_text, ocr_used, quality, warnings = ResumeParserService.parse_pdf(
                file_bytes
            )
            parser = "PyMuPDF"
        elif file_type == "docx":
            raw_text, ocr_used, quality, warnings = ResumeParserService.parse_docx(
                file_bytes
            )
            parser = "python-docx"
        else:
