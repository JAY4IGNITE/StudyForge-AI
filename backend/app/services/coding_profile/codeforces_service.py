...
async def fetch_codeforces_stats(username: str) -> Dict[str, Any]:
    async with httpx.AsyncClient() as client:
        # Fetch user info
        info_resp = await client.get(f"{CODEFORCES_API_URL}/user.info?handles={username}")
        ...
        rating_resp = await client.get(f"{CODEFORCES_API_URL}/user.rating?handle={username}")
