...
def get_embedder():
    ...
    try:
        ...
        logger.info(
            "Initializing SentenceTransformer model (this may take a moment to download)..."
        )
        embedder = SentenceTransformer("all-MiniLM-L6-v2")
    except ImportError:
        embedder = None
        logger.warning(
            "SentenceTransformer not installed. Semantic matching will fallback to basic exact matching."
        )
...
class AtsEngineService:
    ...
    def calculate_keyword_score(
        self, resume: ParsedResume, jd: JobDescriptionSchema
    ...
    def calculate_semantic_score(
        ...
        resume_exp_text = " ".join(
            [e.description or "" + " ".join(e.bullets) for e in resume.experience]
        )
        resume_proj_text = " ".join(
            [p.description or "" + " ".join(p.bullets) for p in resume.projects]
        )
    ...
    def calculate_formatting_score(
        ...
        if resume.parse_quality < 0.8:
            score -= 10.0
            warnings.append(
                "Text extraction problems detected. Resume might have complex layouts or columns."
            )
    ...
    def calculate_impact_score(self, resume: ParsedResume) -> float:
        ...
        if not all_bullets:
            ...
            metrics = re.findall(
                r"(\d+%|\$\d+|\d+[kKmM]|increased|reduced|achieved)",
    ...
    async def _generate_nim_recommendations(
        self,
        resume: ParsedResume,
        jd: JobDescriptionSchema,
        breakdown: AtsScoreBreakdown,
...
"""
        ...
        try:
            ...
            if isinstance(recs, list) and len(recs) > 0:
                return recs[:5]
            return [
                "Review the job description closely and incorporate missing keywords."
            ]
        except Exception as e:
    ...
    async def analyze(
        ...
        recommendations = await self._generate_nim_recommendations(
            resume, jd, breakdown, matched, missing
        )
