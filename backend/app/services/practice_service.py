...
class PracticeService:
    async def create_session(
        self, user_id: str, topic_id: str, target_role: str
    ) -> Dict[str, Any]:
    ...
    async def generate_next_question(
        self, session_id: str, user_id: str, target_role: str
    ) -> Dict[str, Any]:
        session = await self.get_session(session_id, user_id)
        topic = await Topic.get(session.topic_id)
        mastery = await adaptive_service.get_or_create_mastery(
            user_id, session.topic_id
        )
    ...
    async def submit_attempt(
        ...
        updated_mastery = await adaptive_service.update_mastery_after_attempt(
            user_id=user_id, topic_id=session.topic_id, latest_score=eval_res.score
        )
